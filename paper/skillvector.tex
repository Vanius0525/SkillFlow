...
shortcuts
\citep{balepur2024artifacts} (Appendix~\ref{app:traps}). Other domains give limited evidence
...
answer format; what separates it from span injection's $0.89$ is whether the
patched positions are the document's own. We therefore read the low ceiling as
a scope condition on patching positions the document does not occupy, rather
than as a negative result about $\dvec$. In a multiple-choice
...
\paragraph{Which control you subtract chooses what you measure.} $\dvec$ is
defined against a control document, so what ``content'' names depends on which
document is subtracted: against an unrelated one it is topic, procedure and
numbers together, against the same document with its numbers replaced or its
lines shuffled it is much less. The three differ by a factor of three to four
(\S\ref{app:onepos}); Appendix~\ref{app:controls} gives the full comparison
