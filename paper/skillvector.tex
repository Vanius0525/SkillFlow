...
Main span results and the clinical position comparison use the correction;
the synthetic experiments remain \emph{pre-fix}. Appendix~\ref{app:limits} records
...
$0.05$. A same-family mean control recovers $0.12$ on its $41$ eligible items.
...
and $0.92$, respectively, on each rank experiment's own subset. The Mistral
rank subset ($92$ items, $26$ calculators) differs from its newer battery
subset in Table~\ref{tab:replication}. Dashed curves retain bottom directions.
...
alone, whereas layers $12$--$15$ recover $0.81$ ($n=222$). This completed
window experiment uses a different wrong-content receiver and its own
baselines; it is not directly paired with (a). Span interventions use the
corrected implementation; attention knockout does not use state capture.}
...
Mistral's MedCalc rank/depth results retain their earlier completed subsets.
...
states repeatedly within consecutive layer windows. The latest complete
MedCalc window run uses a different receiver from the span battery. Screening
and normalising against its own baselines retains $222$ items from $21$
calculators: windows $8$--$11$, $12$--$15$, $14$--$19$ and $16$--$35$
recover $0.95$, $0.81$, $0.22$ and $0.10$, respectively. The last matches
layer $16$ alone; the identity control gives $0.00$. On TheoremQA ($88$ items,
$72$ groups, also with its own receiver), the same windows recover
...
combinations. The largest available control is $0.35$, for a same-family
mean on $31$ Mistral/MedCalc items; full-span recovery on that task is $0.84$
over $92$ items, so these are not a paired comparison. Wherever both depth comparisons
...
leading directions. The rank knee varies across model families and does not
scale proportionally with width within Qwen. Across layers, states stop
