...
single-position intervention depends on how the answer is expressed: in an
answer-format comparison, recovery is $1.000$ for one-token multiple choice,
...
subtraction separates content from presence, answer-format interventions explain
single-position failures, and controlled span injection identifies where the
skill's effect can be recovered (\S\ref{sec:decomp}--\ref{sec:whose}).
\item \textbf{Measuring the representation rank required for recovery.}
Truncating the injected matrix reveals a need for multiple leading directions,
rejects proportional width scaling in the tested family, and identifies
variation across model families (\S\ref{sec:rank}, \S\ref{sec:ladder}).
\item \textbf{Separating skill transfer from continued reading.} Span
replacement and cumulative attention knockout establish distinct depth
boundaries; layer-window interventions and spectral diagnostics show that lost
transfer is explained by neither too few late injection layers nor the apparent
collapse in representation rank (\S\ref{sec:depth}).
\end{enumerate}
...
The main MedCalc span battery takes every rescued instance of every calculator
with at least two ($467$ items over $48$); receiver screening retains $135$
items from $14$ calculators, and the depth sweep, the truncation curve and the
within-document quarters are read on that same set. The battery was developed on
a sample of four instances from each of forty calculators, which screening cut
to $60$ items from $15$; Appendix~\ref{app:fullscale} reports both.
Development experiments use ten calculators.
...
Writing the receiver's own captured states back exposed a kernel mismatch:
capture omitted an attention mask while decoding supplied one. Before correction,
the null arm changed $108/1{,}766$ Qwen3-8B outcomes ($6.1\%$); matching the mask
made it exact on $80/80$ items across two reruns. Main span results use the
corrected implementation unless marked \emph{pre-fix}; the synthetic and
final-position results predate it. Appendix~\ref{app:limits} records provenance
and Appendix~\ref{app:detail} the implementation. Such numerical discrepancies
\citep{he2025nondeterminism} can propagate through long generations.
...
\label{sec:onepos}
Table~\ref{tab:answerformat} isolates answer format on the synthetic task.
For numeric answers, all final-position arms yield zero through layer $22$;
the best replacement recovers $0.304$, content $0.268$ and presence $0.018$.
Among the $120$ numeric answers, $36$ use two tokens, $55$ three and $29$ four.
Increasing the patched position count from $1$ to $4$ to $16$ gives
$0.286,0.286,0.554$ recovery. With step-by-step reasoning on an $80$-item subset,
the skill raises accuracy from $0.000$ to $1.000$, but a single-position patch
recovers $0.000$. This supports an answer-dependent causal probe, without
establishing a universal information bound on hidden states.

\begin{table}[t]
\caption{One final-position probe across answer formats, Qwen3-8B (pre-fix).
Multiple choice and numeric answers use the same $120$ items; reasoning uses an
$80$-item subset with the same skill. Recovery is the best state-replacement
result over tested layers on each format's rescued cell.}
\label{tab:answerformat}
\centering\small
\input{tab-answer-format}
\end{table}

Clinical skills show the same failure of the compact probe: final-position
replacement repairs at most $3$ of $18$ rescued items at layer $10$ and $0$--$1$
from layer $14$ onward. Question-span transplantation ($50$--$1{,}697$ positions)
repairs none at seven tested layers, with an exact null control
(Appendix~\ref{app:onepos}).
...
which remain within $0.10$ accuracy of their no-skill baseline. Over every
rescued item, recovery is $0.89$ (Table~\ref{tab:big40}). This is a
...
The dose sweep is sharply nonlinear. Over every rescued item ($n=135$ from $14$
calculators), $\alpha=0.4,0.5,0.55$ gives $0.04,0.04,0.14$ recovery, while
$0.6,0.7,1.0$ gives $0.53,0.87,0.89$ and $\alpha=2$ falls back to $0.77$. Most
of the transition lies between $0.55$ and $0.7$, as it did on the
four-calculator development sweep ($0.06,0.00,0.06$ then $0.62,0.94,1.00$,
$n=16$).
...
\begin{table}[t]
\caption{Span interventions over every rescued item ($467$ from $48$
calculators), restricted to the $135$ items from $14$ calculators whose
wrong-skill receiver solves none. Recovery uses
...
The receiver screen matters: on $34$ of the $48$ calculators the wrong skill
already solves some items. Another skill's content recovers $0.17$ there
($n=332$), against $0.05$ on the $14$ restricted calculators ($n=135$), and
norm-matched noise $0.10$ against $0.02$; both are consistent with part of the
apparent transfer releasing knowledge the model already has. The development
set showed the same contrast more sharply ($0.47$ against $0.19$, $n=24$ and
$16$). The forty-calculator restriction is recomputed independently. Its skill
...
Table~\ref{tab:skillcontent} asks what the transferable span represents. The
formula quarter contributes the largest standalone effect ($0.56$), about two
thirds of the full span's $0.89$; other quarters contribute through combination.
With the skill before the question, causal masking makes skill states identical
across instances at equal prompt length (cosine $\ge0.997$ otherwise). Placing
the skill after the question permits question dependence: another instance's
representation transfers almost fully at layer $8$, but less well at $16$,
while the instance-specific remainder remains ineffective alone. The clean
skill--question separation therefore holds at the early injection layer.

\begin{table}[t]
\caption{What the span content carries, Qwen3-8B after the instrument fix.
...
on $151$ items from $17$ calculators. Both halves are the full-dataset rerun. All entries are recovery $\rho$.}
\label{tab:skillcontent}
\centering\small
\input{tab-skill-content}
\end{table}

Two additional controls exclude a common shift or a norm change as sufficient
accounts. Averaging content across skills preserves $54$--$60\%$ of its energy
but recovers only $0.11$ ($0.06$ with the skill last; pre-fix); matching each
position to the receiver's norm leaves recovery at $1.06$ versus $1.06$
(pre-fix). Prompt order also changes transfer at layer $16$: over every rescued
item, $0.49$ with the skill last ($n=151$ from $17$ calculators) against $0.06$
with it first ($n=135$ from $14$); on the development sets the same contrast read
$0.85$ against $0.19$ (Appendix~\ref{app:belongs}).
...
variation is needed. At layer $8$ over every rescued item ($n=135$ from $14$
calculators), recovery is $0.07,0.05,0.12$ at $k=4,16,32$, then $0.63$ at $64$
and $0.77$ at $128$, against $0.89$ untruncated (Figure~\ref{fig:rank}). The
bottom-$16$ and bottom-$64$ controls recover $0.04$ and $0.03$. Useful directions are therefore concentrated
...
has $k^{*}=47$ [$43,55$], or $1.1\%$ of its width. Development and deeper-layer curves are reported in Appendix~\ref{app:rank2}. These are measurements of the injected representation;
...
within Qwen does not yield a proportional change in $k^{*}$ ($40$ versus $47$);
Mistral at width $4096$ has $k^{*}=84$. The dashed control retains bottom
singular directions. Untruncated recovery is $0.89$, $0.89$ and $0.92$,
...
Figure~\ref{fig:transferreading} directly compares the two interventions on
Qwen3-8B. Rerun over every rescued item at one-layer resolution
($n=135$ from $14$ retained calculators, Appendix~\ref{app:fullscale}), span
replacement recovers $0.98$ at layers $0$ and $4$, $0.89$ at $8$ and $0.74$ at
$12$ of $36$, then $0.16$ at $13$ and at $14$, and $0.06$ or below from $15$ to
$20$. The transition occupies a single layer: layers $13$ and $14$ are
indistinguishable and the whole drop falls between $12$ and $13$, which a sweep
at stride $2$ or coarser cannot localise. We call the crossing below $\rho=0.5$
the \emph{handover} boundary. It survives a continuous readout with no grading
threshold in it (Appendix~\ref{app:fullscale}), so it is not an artefact of
scoring a chain of thought right or wrong. By contrast, \emph{cumulative attention knockout}
blocks attention to skill tokens at every layer from $L$ onward. On all $466$
rescued items it leaves only $0.10$--$0.18$ correct for $L\le20$, except $L=4$
($0.23$), then $0.46$ at $24$, $0.79$ at $28$ and $0.91$ at $32$. Skill attention
remains important to about $0.78$ of model depth, long after transfer has
collapsed. Continued necessity therefore does not imply sufficiency for a late
transplant. The panels use separate evaluation sets and distinct readouts.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig-transfer-reading.pdf}
\caption{Skill transfer ends before continued reading, Qwen3-8B on MedCalc.
All three panels are the full-dataset rerun: every rescued item of every
calculator with at least two. (a) Single-layer span replacement, at one-layer
resolution through the handover: recovery relative to the donor--receiver
accuracy gap, $n=135$. The shaded band is the single layer, $12$ to $13$, in
which the whole drop occurs; a sweep at stride $2$ or coarser cannot localise
it. (b) Cumulative attention knockout: fraction of rescued items still correct
when skill attention is blocked from $L$ onward, $n=466$.
The dotted line marks $0.5$ in each panel's own metric. (c) Injecting into twenty
late layers ($16$--$35$) gives no gain over layer $16$ alone, and a window
beginning at $14$ is no better; four earlier layers
($12$--$15$) recover $0.74$, $n=137$. Span interventions use the corrected
implementation; knockout does not use the capture path.}
...
layers matter, we patch only within fixed windows. Over every rescued item
($n=137$ from $15$ calculators), layers $8$--$11$ recover $0.91$,
$12$--$15$ recover $0.74$, $14$--$19$ recover $0.10$, and $16$--$35$ recover
$0.10$, matching the single-layer-$16$ result in the same run ($0.10$); the null
arm is $0.00$. A window that begins at $14$ is therefore already as useless as
one that begins at $16$, which is what a boundary between $12$ and $13$
predicts. On TheoremQA, the same windows yield $0.90,0.73,0.45,0.21$, with the
...
$0.02,0.21,0.74,0.72$, giving $k^{*}=40$ [$32,50$] against Qwen3-8B's $47$
...
\label{sec:traps}
Decoded accuracy is essential. Aggregate gold-answer log-probability can favour
a content-free mean by $3.75$ nats despite lower accuracy ($0.256$ versus
$0.436$); a forced \texttt{ANSWER:} cue removes the skill's behavioural effect
while retaining a $+0.60$ nat difference. Multiple choice also retains nearly
three quarters of a skill's effect after its numbers are corrupted
\citep{balepur2024artifacts}. Appendix~\ref{app:traps} gives these diagnostics and
the wrong-skill baseline comparison. Additional-domain checks remain in
Appendix~\ref{app:replication}: LogicBench's binary answers yield high control
recovery, CHAMP has only $20$ rescued items, and ToolQA/BigCodeBench require
execution outside this no-tool setting.
...
the weaker one. No control exceeds $0.12$ under either.}
\label{tab:fullscale}
\centering
\small
\begin{tabular}{lccc}
...
Rerun over every rescued item ($n=137$ from $15$ calculators), the windows read
$0.91$ for $8$--$11$, $0.74$ for $12$--$15$, $0.10$ for $14$--$19$ and $0.10$ for
$16$--$35$, against a null arm at exactly $0.00$. Four layers carry most of the
effect if they are layers $12$ to $15$, and none of it if they start two layers
later: on the forty-calculator subset the $14$--$19$ window read $0.38$, and at
full scale it is at the receiver. Twenty layers are worth almost nothing if they start at $16$. Widening the window from one layer to
