...
from $1.000$ for a one-token answer to $0.304$ for a short number and $0.000$
...
model width within Qwen and also varies across model families. Across two task
...
$[-8.8,+3.0]$). Yet in a synthetic task, correct and wrong skills produce
...
$0.062$ of the correct skill's gain, whereas the correct-minus-wrong difference
recovers $0.812$. Recovery also depends on answer format: the best state
replacement recovers $1.000$ for one-token multiple choice, $0.304$ for a short
number and $0.000$ for step-by-step reasoning. Success at the final prompt
...
this value from $47$ to $40$, rather than the pre-registered proportional
prediction of approximately $12$; Mistral at the larger width gives $64$--$84$.
...
within Qwen and vary across model families (\S\ref{sec:rank}, \S\ref{sec:ladder}).
...
one question. We report sample sizes for each comparison. Recovery intervals
in the replication table and intervals for the rank-curve summary resample
skill groups. Other accuracy intervals and plot error bars use item-level
estimates, as specified in their captions, and do not account for dependence
among questions sharing a skill.
...
Matching the masks restored exact agreement on $80/80$ items across two reruns.
Main span results use the correction unless marked \emph{pre-fix}; synthetic
and final-position experiments predate it. Appendix~\ref{app:limits} records
...
On the synthetic task with Qwen3-1.7B, we inject each component of
Eq.~\ref{eq:decomp} at the final prompt position. On rescued items, the
presence component recovers $0.062$ of the gain, matching the wrong-skill
baseline; the content component recovers $0.812$, compared with $1.000$ for
full state replacement. The effect extends to failures: across rescued,
persistent-failure, kept and broken items, replacement gives accuracies of
$1.000/0.000/1.000/0.000$, while content injection gives
$0.812/0.112/1.000/0.125$. Content injection thus broadly reproduces both the
benefits and the errors induced by the skill. Recovery also depends on the
subtraction baseline. Using a skill with corrupted numbers gives
$0.23$--$0.31$, and using shuffled sentences gives $0.19$--$0.29$, compared
with $0.812$ for an unrelated skill. The content component must therefore be
...
For short numeric answers, none of the final-position interventions recovers
any gain through layer $22$. The best results across tested layers are $0.304$
for state replacement, $0.268$ for content and $0.018$ for presence.
These answers are only two to four tokens long ($36$, $55$ and $29$ of the
$120$ answers, respectively). In a separate position-count comparison,
patching $1$, $4$ and $16$ positions recovers $0.286$, $0.286$ and $0.554$.
For step-by-step reasoning on an $80$-item subset, the skill raises accuracy
from $0.000$ to $1.000$, yet a single-position patch recovers $0.000$.
...
\begin{table}[t]
\caption{Final-position state replacement across answer formats, Qwen3-8B (pre-fix).
...
result over tested layers on each format's rescued items.}
\label{tab:answerformat}
\centering\small
\input{tab-answer-format}
\end{table}

The clinical tasks show a similar limitation. Final-position replacement
corrects at most $3$ of $18$ rescued items at layer $10$, and only $0$--$1$
at tested layers from $14$ onward. Replacing states across the question span
($50$--$1{,}697$ positions) corrects none at seven tested layers, even though
the identity control is exact (Appendix~\ref{app:onepos}). Increasing the
number of patched positions alone is therefore insufficient in this setting.
...
$8$, span injection raises accuracy from $0.00$ to $1.00$ on the screened
development subset ($n=16$). Figure~\ref{fig:channels} places this result
alongside final-position interventions, which remain within $0.10$ accuracy
of their no-skill baseline; the two panels use different receivers and subsets.
In the larger rerun, span injection recovers $0.89$ on the screened $135$-item
subset (Table~\ref{tab:big40}). In each case, the intervention changes only
the skill span at one layer of the wrong-skill receiver.

\begin{figure}[t]
\centering
\includegraphics[width=0.76\textwidth]{fig-channels-medcalc.pdf}
\caption{Interventions at the final prompt position and across the skill
span on clinical tasks with Qwen3-8B. Top: each arm's best accuracy over layers
...
increase occurs between $0.55$ and $0.7$, consistent with the development
sweep ($n=16$). To test content specificity, we compare correct and other-skill
injections over a common prefix length. In the development comparison, these
recover $0.94$ and $0.19$, respectively. In the larger rerun, the correct
prefix recovers $0.65$, whereas content from an unrelated skill recovers
$0.05$. Permuting positions also gives $0.05$, norm-matched Gaussian noise
...
by requiring the wrong-skill receiver to solve no sampled item in a group. Recovery uses
...
The development comparison shows the same pattern for other-skill content
($0.47$ versus $0.19$, $n=24$ and $16$). A separate caveat is intervention
size: the earlier forty-calculator sample has skill spans of
$415$--$1{,}630$ positions, far more than a single-position probe.
...
otherwise (cosine $\ge0.997$). Placing the skill after the question allows
...
Two earlier controls further test whether a shared shift or a change in state
norm could explain recovery. Averaging content differences across skills
preserves $54$--$60\%$ of their energy but recovers only $0.11$ ($0.06$ with
the skill last). Rescaling patched states to match the receiver's norm at each
position leaves recovery unchanged at $1.06$. Both results predate the fix
and should be read with that qualification. In the corrected rerun, layer-$16$
...
$128$, compared with $0.89$ without truncation (Figure~\ref{fig:rank}).
Keeping the bottom $16$ or $64$ directions recovers only $0.04$ or $0.03$.
...
Qwen3-8B have similar rank knees ($40$ and $47$); Mistral-7B, with the same
width as Qwen3-8B, has $k^{*}=84$. Untruncated recovery is $0.89$, $0.89$
and $0.92$, respectively. The dashed control retains bottom singular
directions. Error bars use item-bootstrap intervals scaled by the observed
donor--receiver accuracy gap.}
\label{fig:rank}
\end{figure}
...
alone; starting at $14$ is similarly ineffective. In contrast, layers
...
states repeatedly within consecutive layer windows. An earlier pre-fix run
recovered $0.950$ when patching every layer. In the corrected MedCalc run
($137$ screened items, $15$ calculators), windows $8$--$11$ and $12$--$15$
recover $0.91$ and $0.74$, whereas $14$--$19$ and $16$--$35$ both recover
only $0.10$. The latter matches layer $16$ alone in the same run; the identity
control gives $0.00$. On TheoremQA, the same windows recover
$0.90,0.73,0.45,0.21$, with the last again matching layer $16$ alone.
Repeated late injection therefore does not restore early-layer recovery in
these tests. One possible explanation is that later reading depends on
...
is distributed across singular directions. It falls from $64$ at layer $12$
to $17$ at layer $16$, but this drop is driven by unusually large activations
\citep{sun2024massive, shi2026melayer}. In $34$ of $40$ skills, one token's
state reaches $99$--$180$ times the median span norm, dominated by the same
two hidden dimensions. Removing the four highest-norm positions from roughly
four hundred restores the ratio to $66$ and makes it approximately flat over
layers $0$--$28$. The drop is also absent in Qwen3-0.6B and Qwen3-1.7B.
...
intervals resample skill groups. Max control summarises the available
half-dose, other-skill, permutation and noise comparisons on the main subset.
...
items. $k^{*}$ is the rank knee on the fixed grid. ``--'' marks a measurement
not run after the fix.}
...
combinations. The reported controls remain at or below $0.27$, less than a
third of the corresponding full-span recovery. Wherever both depth comparisons
are available, the transfer boundary occurs before the reading boundary.
The measured transfer boundaries lie at $0.33$--$0.43$ of model depth within
Qwen and at $0.19$--$0.22$ in Mistral. Reading boundaries also vary by task:
Qwen3-8B reaches the defined threshold at $0.56$ on TheoremQA and $0.78$ on
MedCalc, while Mistral reaches it at $0.62$ and $0.88$, respectively.
...
$0.02,0.21,0.74,0.72$, giving $k^{*}=40$ [$32,50$], close to Qwen3-8B's
$47$ [$43,55$] despite the fourfold width difference. On TheoremQA, the
corresponding knees are $52$ and $50$, again inconsistent with proportional
scaling. Their participation ratios are also similar in scale ($66$--$77$
versus $70$--$99$ over layers $0$--$20$, excluding unusually large
activations). Mistral, with the same width as Qwen3-8B, instead gives
$k^{*}=84$ [$59,97$] on MedCalc and $64$ [$50,79$] on TheoremQA. The
between-family difference is clearer on MedCalc. Because Mistral's recovery
at $k=128$ remains below the untruncated result ($0.71$ versus $0.92$), its
knee summarises only the tested range and may change on a wider grid.
These comparisons show that the measured rank requirement is not determined
by hidden width alone.
...
In the evaluated tasks and screened subsets, hidden states across a skill's
own tokens transfer much of its behavioural effect. A one-time intervention at
the final prompt position is more sensitive to answer format and can succeed
for a compact answer while failing for extended reasoning. Comparisons with
wrong skills, reordered states and truncated representations show that span
recovery depends on content, position and multiple leading directions. The
rank knee varies across model families and does not scale proportionally with
width within Qwen. Across layers, span states stop transferring most of the
effect before attention to them ceases to support performance; the observed
spectral drop does not establish compression at this boundary. These results
support studying skill representations through both what their states can
transfer and how subsequent computation continues to use them.
