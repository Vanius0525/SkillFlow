...
the last prompt position lands on the no-document baseline: the content component
$\dvec = h_{\text{skill}} - h_{\text{wrong-skill}}$, the presence component
$\gvec$, donors from other instances and other skills, and replacing the entire
state with the with-document one---five arms, each at its best layer, all within
$0.10$ of a model given no document at all, against $0.95$ for the document
itself. The content is not absent from that position: it identifies which of $41$
skills is present at $0.93$--$1.00$ against a chance rate of $0.024$. What fails
is the channel. Holding the model, the document and the items fixed and changing
only the answer format, a single-position patch recovers $1.000$ of the rescued
items for a one-token answer, $0.30$ for a short number, and $\mathbf{0.000}$
when the same items are answered by reasoning step by step. One position carries
the model's decision, and a decision spread over hundreds of tokens does not fit.
We then find the channel that does carry: the document's own token span.
Substituting it, length-matched so no position changes its rotary phase, takes
the rescued items from $0.00$ to $0.95$. Because a length-matched receiver
already holds a document, that substitution \emph{is} content injection at
$\alpha=1$ spread over $m$ positions---so the whole vector battery becomes
available on real skills. On the calculators where the document is
load-bearing---the fifteen of forty calculators a wrong document never
rescues, $59$ items---it behaves like a mechanism and every control is at the
floor: $\rho = 0.87$ at $\alpha=1$ against $0.13$ at $\alpha=0.5$, $0.04$ for a
donor from another skill family, $0.12$ under permutation of the span's own
positions and $0.13$ for norm-matched noise, against a null arm at $0.04$. What carries is not a common
shift---averaging over every skill recovers $+0.06$ while accounting for $58\%$
of the vector's energy---and it is not a joint encoding of skill and task:
another instance's vector recovers as much as the instance's own ($1.06$ against
$1.00$) and the instance-specific remainder recovers nothing. It is, instead,
high-rank, and we measure how high by truncating it before injecting it:
recovery is at the receiver's floor for every rank up to $16$, reaches $0.72$ at
$64$ and $0.94$ at $128$, while the discarded bottom $64$ directions recover
nothing. That number does not move---not with depth, and not with model width:
on Qwen3-0.6B, whose residual stream is four times narrower, the knee sits
between $32$ and $64$ exactly as it does on the 8B, against a width-proportional
prediction of $12$, registered before the run. A document therefore costs about
$4\%$ of a $1024$-wide residual stream and $1.2\%$ of a $4096$-wide one. These
results relocate the known limit on what one injected vector can do: it is not
only a limit on the mapping the vector must express---a $39$-row lookup the
model has never seen passes through a single position at $0.812$ when the answer
is one token---but a limit on the output it must produce. Along the way we
retract a mechanism of our own: the span's participation ratio appears to
collapse from $64$ to $17$ exactly where the handover dies, and that collapse is
massive activations, not compression---four positions out of four hundred
removed, it is flat.
\end{abstract}
...
\section{Which component does the work}
\label{sec:decomp}
...
every position's attention to the document from layer $L$ to the end costs
$0.500 \to 0.050$ for $L \in \{0, 6, 12, 18\}$, $0.100$ at $L=24$ and
$\mathbf{0.300}$ at $L=30$. Blocking only the last six of $36$ layers still
costs two fifths of the effect: the model is still reading the document long
after a single-layer handover has stopped working.
...
A skill document's effect on a language model is not carried by a vector at the
position the model answers from. On tasks whose answers are longer than a token,
that position holds the model's decision, and a decision that occupies hundreds
of tokens does not fit into one state---which is why every arm we write there,
including a wholesale replacement, lands on the no-document baseline, and why
the effect reappears as soon as the answer is made short.

It is carried by the document's own tokens, and there the same subtraction
behaves like the mechanism the literature has been looking for: complete at
$\alpha=1$, thresholded between $0.55$ and $0.6$ of a dose, indifferent to
the norm of what is written, destroyed by permuting the span's positions, and
absent for a donor from another skill against a position-matched control. What
it is made of is not a common ``a document is present'' shift---averaging over
every skill carries nothing, though that average is more than half of the
vector's energy---and it is the skill rather than the skill and the task
together. In the format these documents are actually used in, causal masking
makes the span bit-identical across a skill's instances, so the vector that
reproduces the whole effect cannot contain any task information; placed where it
\emph{could} contain some, another instance's vector still works as well as the
instance's own, and the instance-specific remainder works not at all.
...
fit into, or a patch placed where the document itself is.
