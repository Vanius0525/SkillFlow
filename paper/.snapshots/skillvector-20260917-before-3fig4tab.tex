...
\begin{abstract}
Skills supplied in a language model's context can substantially improve task
performance, but how their content is represented during computation remains
unclear. Is a skill compressed into one vector, or does its effect require
representations distributed across token positions? We study this question by
separating the state change associated with a skill's presence from the change
associated with its content, using a length-matched wrong skill as a control.
Although presence dominates the geometry at the final prompt position, content
drives the behavioural improvement. Changing the answer format reveals why
patching that position gives an incomplete account: recovery falls from $1.000$
for a one-token answer to $0.304$ for a short number and $0.000$ for a chain of
thought. Injecting content over the skill's own token span instead recovers
$0.87$ of its effect on the restricted split of a forty-calculator evaluation,
while half dose, another skill, permuted positions and noise each recover at
most $0.10$. Rank truncation shows that this effect requires multiple leading
directions: recovery is $0.13$ at rank $32$ and $0.77$ at rank $128$, with no
recovery from the bottom $64$ directions. The rank-curve knee does not scale
proportionally with model width within the tested family, but differs across
families. Replication on two task domains and two model families further shows
that successful span injection ends earlier in depth than the model's dependence
on attention to that span. These results distinguish a transferable skill
representation from a single-position representation of an impending answer.
\end{abstract}
...
that such skills can improve task performance \citep{su2026srabench,
skillsbench2026}, but a higher score does not explain how the model uses the
skill internally. A central question is whether the model compresses the skill
into a single vector, represents it across several token positions, or continues
to depend on its full token span. Answering this question requires connecting
internal representations to the performance they can causally recover, and
measuring how many directions those representations need.

We begin with a paradox: \emph{skill content matters strongly for behaviour,
yet much of the observed state change does not distinguish the right skill from
a wrong one}. Across $1{,}100$ clinical-calculator instances, the correct skill
improves accuracy by $39.7$ percentage points over no skill, whereas a
length-matched wrong skill changes it by $-2.9$ points ($95\%$ interval
$[-8.8,+3.0]$). In a synthetic control, however, the state displacements caused
by the correct and wrong skills at the final prompt position have cosine
similarity $0.91$--$0.95$ over layers $0$--$10$. Thus a large, shared geometric
shift coexists with a sharply content-dependent behavioural effect. Does the
shared shift carry the improvement, or does a smaller content-specific component
do the work?

The decisive experiment separates these components and injects each back into
the model. On the synthetic control, the shift associated with skill presence
recovers $0.062$ of the rescued items, exactly what the wrong skill achieves,
whereas the content component recovers $0.812$. Geometry alone therefore misses
the component responsible for most of the improvement. Yet this successful
single-position intervention depends on how the answer is expressed: on a
matched format comparison, recovery is $1.000$ for one-token multiple choice,
$0.304$ for a short numeric answer and $0.000$ for a chain of thought. The
result points to an answer-dependent probe: the final prompt position can
transfer a compact decision, while failing to reproduce the skill's support for
an extended computation.

Our core discovery is that \emph{the skill's own token span provides a causal
route for transferring its effect}. We replace the skill with a wrong skill of
the same token length, then inject the difference between their hidden states
over those positions. At unit dose this is exactly a transplant of the correct
skill's span states at one layer. On forty clinical calculators, the intervention
recovers $0.87$ of the skill's effect on the restricted split, while half dose,
another skill, permuted positions and norm-matched noise remain at or below
$0.10$. Truncating this content matrix then measures the representation needed
for recovery: $32$ leading directions recover $0.13$, $128$ recover $0.77$, and
the bottom $64$ recover nothing. The tested representation therefore retains
both positional structure and multiple useful directions.

Further experiments establish how this finding depends on the model and its
computation. Reducing width fourfold within the Qwen family moves the rank-curve
knee from $52$ to $40$, far above the pre-registered width-proportional prediction
of $12$; Mistral at the larger width instead needs $64$--$84$ directions across
two tasks. The span remains necessary to attend to after its states cease to
transfer most of the effect into a receiver. This depth separation replicates
where both interventions are available, and an apparent drop in representation
dimensionality turns out to be caused by massive activations rather than skill
compression. Our contributions are:
\begin{enumerate}\itemsep0pt\parskip0pt
\item \textbf{Separating presence, content and answer format.} A matched-control
subtraction and format comparison explain why large state changes and successful
single-position patches need not identify a transferable skill representation
(\S\ref{sec:decomp}).
\item \textbf{Identifying a span for causal skill injection.} Injection over a
skill's own positions recovers its effect under dose, donor, order and noise
controls on externally authored skills (\S\ref{sec:battery}).
\item \textbf{Measuring the rank needed for behavioural recovery.} Truncation of
the injected representation reveals a need for multiple leading directions,
rejects proportional width scaling in the tested family, and identifies
variation across families (\S\ref{sec:rank}, \S\ref{sec:ladder}).
\item \textbf{Separating transfer depth from continued use.} Span replacement,
layer-window interventions and attention knockout distinguish where skill states
can be transferred from where they remain necessary, while spectral controls
rule out the apparent compression explanation (\S\ref{sec:depth}).
\end{enumerate}
...
procedures \citep{su2026srabench, skillsbench2026, sweskills2026}. Our question
concerns the internal representation after a skill has been supplied.
Task and function vectors show that a compact activation can transfer behaviours
from in-context demonstrations \citep{hendel2023taskvectors,
todd2024functionvectors}; task arithmetic studies analogous edits in weight
space \citep{ilharco2023taskarithmetic}.
\citet{dong2026taskvectorlimits} analyse restrictions on the task mappings such
vectors can express, including failures on high-rank mappings. We distinguish
this mapping rank from the rank of a skill's representation across positions,
and from the answer format under which that representation is tested.
Learned activation aggregation \citep{ardoin2026promptcompression}, gist tokens
\citep{mu2023gist} and context autoencoders \citep{ge2024icae} explicitly train
compression mechanisms. We instead intervene on states produced by the original
model without training a compressor. Context distillation into weights
\citep{doc2lora2026, deepcontextdistill2025} addresses a complementary route.

\paragraph{Answer representations and activation interventions.}
Multiple-choice answering assembles a decision about an answer symbol
\citep{wiegreffe2025mcq}, while reasoning involves effects distributed over
generated positions \citep{dura2026sequentialpatching}. Patching from a completed
reasoning trajectory \citep{mehrafarin2026cothidden} also differs from extracting
states before reasoning begins, as we do here. Directions applied repeatedly
can steer extended generations \citep{panickssery2024caa, liu2024icv,
stolfo2025instruction}; our single-position result concerns a one-time write
at a prompt position. Following activation-patching methodology
\citep{zhang2023bestpractices, heimersheim2024patching}, we use explicit receiver
baselines, behavioural outcomes and identity interventions. Prior tracing
localises factual information and context use across layers
\citep{meng2022rome, geva2023dissecting, sia2024incontexttranslation,
onset2025instruction}. We compare span replacement and attention knockout to
test whether transfer and continued use end at the same depth.

\paragraph{Representation rank and spectral measurements.}
Spectral studies characterise hidden-state geometry
\citep{valeriani2023geometry}, causal sensitivity through Jacobians
\citep{sarkar2026causaldim}, and operators composed across layers
\citep{fernando2026residualspectral}. These are different objects from the
position-by-feature content matrix whose rank we truncate before injection.
Massive activations \citep{sun2024massive, shi2026melayer} and outlier dimensions
\citep{timkey2021rogue, kovaleva2021bertbusters} motivate checking whether a
spectral change reflects useful information or a few dominant coordinates.
Our behavioural rank intervention and outlier diagnostics address these two
...
matched to $s$ in token length. For a fixed autoregressive model of hidden width
$D$, let $h^L_{\mathrm{skill}}(p)$, $h^L_{\mathrm{wrong}}(p)$ and
$h^L_{\mathrm{no}}(p)$ denote residual states at layer $L$ and prompt position
...
\label{eq:decomp}
\end{equation}
Layer and position indices are omitted for readability. This is an algebraic
identity: ``presence'' and ``content'' name contrasts defined by the chosen
wrong skill, rather than assuming independent latent factors. Adding each
component to a no-skill run tests its behavioural effect.

For the skill's $m$ token positions $S$, the corresponding content representation
...
\label{eq:inject}
\end{equation}
At $\alpha=1$ this equals the donor's span states. All other positions retain
their receiver states at the intervention layer, and computation then continues.
Token-length matching preserves positions and rotary phases. A final-position
vector and a span matrix use the same subtraction protocol, but are different
representations with different intervention sizes.

\paragraph{Behavioural recovery and evaluation cells.}
We measure generated-answer accuracy and report the recovered fraction
\begin{equation}
\rho(\mathrm{arm})=
...
\label{eq:rho}
\end{equation}
The donor baseline is the correct skill in the prompt, so $\rho=1$ means
recovering its observed gain; $\rho$ can exceed one. We distinguish rescued
...
correct without, wrong with). Main causal comparisons use R and, for span
interventions, retain skill groups whose receiver solves none of the sampled
items. This screen is recomputed within each run, model and task; it does not
select on intervention outcomes. It isolates skills needed relative to the
actual receiver, while defining a conditional evaluation rather than a
population-wide accuracy estimate.

\paragraph{Rank and depth as intervention variables.}
To ask whether the effect fits into a small representation, we truncate the
centred span matrix while preserving its mean over positions:
\begin{equation}
...
\label{eq:rank}
\end{equation}
Here $\bar{\dvec}\in\mathbb{R}^{1\times D}$ is broadcast across positions and
singular values are ordered from largest to smallest. Thus $k$ counts retained
\emph{centred} directions; the injected matrix has rank at most $k+1$. A
bottom-$k$ control retains the same number of trailing directions and the same
mean. We also vary the intervention layer: span replacement tests sufficiency
for transfer, whereas blocking attention to the span from a layer onward tests
continued dependence on it. These interventions answer different causal
questions.
...
The primary material is MedCalc-Bench \citep{khandekar2024medcalc} paired with
externally authored SRA-Bench skills \citep{su2026srabench}: $55$ calculators
with $20$ instances each, totalling $1{,}100$ items. Qwen3-8B
\citep{yang2025qwen3} answers with a chain of thought, using greedy decoding,
without tools, and is graded by the benchmark scorer. Correct skill, no skill
and wrong skill achieve $74.2\%$, $34.5\%$ and $31.5\%$ accuracy on these same
items. A synthetic unit-conversion task supplies a controlled lookup table and
matched negative skills for the decomposition and answer-format experiments.
We additionally use Qwen3-0.6B, Qwen3-1.7B and Mistral-7B-Instruct-v0.3
\citep{jiang2023mistral}, and replicate on SRA-Bench's TheoremQA subset
($747$ questions, $320$ skills), with its prompt and scorer.

\paragraph{Sampling and uncertainty.}
The main MedCalc span battery samples four rescued instances from each of forty
calculators; receiver screening retains $60$ items from $15$ calculators.
The depth sweep samples three per calculator and retains $60$ from $20$;
these are distinct evaluation sets. Development experiments use ten calculators.
TheoremQA uses the same group-based receiver screen, reducing to an item-level
screen for singleton skills. Sample sizes and available measurements are given
with each result. The replication table and rank-knee intervals use bootstrap
resampling over skill groups. The original battery accuracy intervals and plot
error bars use item-level estimates, identified in their captions; these do not
account for dependence among questions sharing a skill.

\paragraph{Identity control and numerical consistency.}
Writing the receiver's own captured states back must reproduce the receiver.
This control exposed an attention-kernel mismatch: capture omitted an explicit
attention mask while decoding supplied one. Before correction, the null arm
changed $108$ of $1{,}766$ Qwen3-8B outcomes ($6.1\%$), despite agreement on all
$360$ paired cells across two machines. Supplying the mask in both passes made
the null arm exact on $80/80$ items across two reruns. Main span results use the
corrected implementation unless marked \emph{pre-fix}; the synthetic and
final-position results predate it. Appendix~\ref{app:limits} records the run
provenance and Appendix~\ref{app:detail} the implementation. This control is
particularly relevant when kernel-dependent numerical differences
\citep{he2025nondeterminism} propagate through a long generation.
...
On the synthetic task with Qwen3-1.7B, we inject each term of
Eq.~\ref{eq:decomp} at the final prompt position. Presence recovers $0.062$ of
the rescued cell, exactly the wrong-skill baseline; content recovers $0.812$
against $1.000$ for full state replacement. The intervention also reproduces
the skill's failures: across R/F/K/B, replacement yields
$1.000/0.000/1.000/0.000$ and content injection
$0.812/0.112/1.000/0.125$. Thus the content effect follows the skill's
behavioural signature, including broken items. The contrast depends on the
negative skill: replacing its numbers lowers recovery to $0.23$--$0.31$, and
shuffling its sentences to $0.19$--$0.29$, compared with $0.812$ for the unrelated
skill. The subtraction control must therefore be specified.

\paragraph{Single-position recovery depends on answer format.}
\label{sec:onepos}
For the same synthetic task, free-form numeric answers yield zero recovery
through layer $22$, even under replacement. The best single-position intervention
recovers $0.304$, with content at $0.268$ and presence at $0.018$; the same model
and items in multiple-choice form reach $1.000$. None of the $120$ numeric gold
answers is one token: $36$ use two tokens, $55$ three and $29$ four. Increasing
the patched position count from $1$ to $4$ to $16$ gives recovery
$0.286,0.286,0.554$, supporting a role for intervention capacity.

In the matched answer-format comparison, prompting the same $80$ items to reason
step by step changes the skill baseline from $0.000$ to $1.000$, yet the
single-position patch recovers $0.000$. The rescued cells across multiple choice,
short numbers and reasoning contain $44$, $56$ and $80$ items, with recovery
$1.000$, $0.304$ and $0.000$. This supports interpreting the one-time patch as
transferring a compact decision; it does not establish a universal information
bound on hidden states. On clinical skills, final-position replacement repairs
at most $3$ of $18$ rescued items at layer $10$ and $0$--$1$ from layer $14$
onward. Transplanting the question span ($50$--$1{,}697$ positions) into a
length-matched receiver repairs none at seven tested layers, with an exact null
control (Appendix~\ref{app:onepos}).

\subsection{The skill span supports content injection}
\label{sec:battery}

\paragraph{Changing the intervention positions restores recovery.}
We now inject the content matrix over the skill's own positions using
Eq.~\ref{eq:inject}. On the development set, this takes the restricted items
from $0.00$ to $1.00$ accuracy at layer $8$ ($n=16$).
Figure~\ref{fig:channels} contrasts this result with final-position interventions,
which remain within $0.10$ accuracy of their no-skill baseline. On the larger
forty-calculator battery, recovery is $0.87$ (Table~\ref{tab:big40}). This is a
single-layer intervention into a receiver containing a wrong skill, with all
positions outside the skill span initially left in the receiver state.

\begin{figure}[t]
\centering
\includegraphics[width=0.76\textwidth]{fig-channels-medcalc.pdf}
\caption{Content interventions at the final prompt position and over the skill
span, on clinical skills with Qwen3-8B. Top: each arm's best accuracy over layers
$10,14,20,30$ ($n=20$, pre-fix), with a no-skill baseline. Bottom: span injection
at layer $8$ ($n=16$, corrected implementation), restricted to calculators whose
wrong-skill receiver solves no sampled item. Panels have different receivers
and evaluation subsets; their baselines are shown separately. Error bars are
item-bootstrap $95\%$ intervals.}
\label{fig:channels}
\end{figure}

\paragraph{Dose, transfer, order and noise controls.}
The development dose sweep is sharply nonlinear: $\alpha=0.4,0.5,0.55$ gives
$0.06,0.00,0.06$ recovery, while $0.6,0.7,1.0$ gives $0.62,0.94,1.00$ ($n=16$,
four calculators). Most of the transition lies between $0.55$ and $0.7$.
On a length-matched prefix of two skill spans, another skill's content recovers $0.19$
against $0.94$ for the correct skill on the same positions. In the larger
battery, another skill gives $0.06$, whether it computes the same quantity or a
different one, against $0.73$ for the correct content on the length-matched prefix.
Permuting positions yields $0.08$ and norm-matched Gaussian noise $0.10$, with
the null arm at $0.00$. Hence successful injection depends on the correct content
and its arrangement across positions, beyond the presence or size of a perturbation.

\begin{table}[t]
\caption{Span interventions on forty calculators, restricted to $60$ items from
$15$ calculators whose wrong-skill receiver solves none. Recovery uses
Eq.~\ref{eq:rho}. Accuracy intervals retain the original item-level normal
approximation; the group-bootstrap interval for transplant recovery is reported
in Table~\ref{tab:replication}. The final rows are identity and unpatched controls.}
\label{tab:big40}
\centering
\small
\input{tab-big40}
...
The receiver screen matters: on six of ten development calculators the wrong
skill already solves some items. Another-skill injection recovers $0.47$ there
($n=24$), against $0.19$ on the restricted calculators ($n=16$), consistent with
part of the apparent transfer releasing knowledge already available to the
model. The forty-calculator restriction is recomputed independently. Its skill
spans contain $415$--$1{,}630$ positions, providing a substantially larger
intervention than one final-position vector.

\subsection{Rank required to recover the skill's effect}
\label{sec:rank}

The span intervention lets us measure how many directions support recovery by
injecting Eq.~\ref{eq:rank}. Every truncated arm retains a single skill-specific
mean vector at all span positions; the test concerns what additional positional
variation is needed. At layer $8$ on forty calculators ($n=60$), recovery is
$0.13,0.06,0.13$ at $k=4,16,32$, then $0.54$ at $64$ and $0.77$ at $128$,
against $0.87$ untruncated (Figure~\ref{fig:rank}). The bottom-$16$ and bottom-$64$
controls recover $0.06$ and $0.00$. Useful directions are therefore concentrated
near the top of the spectrum; preserving the mean and a few directions is
insufficient.

We summarise the curve by $k^{*}$, the first rank reaching halfway from recovery
at $k=16$ to the maximum on the tested grid $\{16,32,64,128\}$, interpolated in
$\log_2 k$. This is a curve knee, not a minimum rank for full recovery. Qwen3-8B
has $k^{*}=52$ [$43,72$], or $1.3\%$ of its width. The ten-calculator development
curve (pre-fix) adds $k=1,2,8$, with recovery $0.11,0.06,0.11$ and $k^{*}=45$.
Deeper development runs at layer $12$ and, with the skill after the question,
layer $16$ place both knees between $64$ and $128$ (pre-fix, $n=16$ and $28$;
Appendix~\ref{app:rank2}). These are measurements of the injected representation;
a truncated intervention can also activate pathways absent in an unpatched run
\citep{makelov2024illusion}.

\begin{figure}[t]
\centering
\includegraphics[width=0.64\textwidth]{fig-rank-medcalc.pdf}
\caption{Behavioural recovery after retaining $k$ leading centred directions,
with the mean preserved, on MedCalc after the instrument fix. Models are
injected before their respective transfer boundaries. A fourfold width change
within Qwen does not yield a proportional change in $k^{*}$ ($40$ versus $52$);
Mistral at width $4096$ has $k^{*}=84$. The dashed control retains bottom
singular directions. Untruncated recovery is $0.87$, $0.89$ and $0.92$,
respectively. Error bars retain the original item-bootstrap intervals, scaled
by the observed donor--receiver accuracy gap.}
\label{fig:rank}
\end{figure}
...
On the forty-calculator depth sweep ($n=60$ from $20$ retained calculators),
single-layer span replacement gives $\rho=1.04,0.94,0.85,0.77$ at layers
$0,4,8,12$ of $36$, falling to $0.28$ at $14$, $0.19$ at $15$ and $16$, and
$0.15$ at $20$. We call the crossing below $\rho=0.5$ the \emph{handover}
boundary: most of the effect can no longer be transferred through these states.

Cumulative attention knockout gives a later boundary. On $40$ rescued items,
blocking attention to the skill from layer $L$ onward leaves only
$0.08$--$0.15$ correct for $L\le20$, except $L=4$ ($0.33$), then $0.38$ at
$24$, $0.75$ at $28$ and $0.95$ at $32$. Thus skill attention remains important
around $0.7$ of model depth, although single-layer transfer has declined by about
$0.4$. The two curves use their respective evaluation sets and test different
operations; continued necessity does not imply sufficiency for a late transplant.

\paragraph{Adding late intervention layers does not restore transfer.}
Patching every layer gives $0.950$ recovery in a pre-fix run. To identify which
layers matter, we patch only within fixed windows. On twenty calculators
($n=48$, corrected implementation), layers $8$--$11$ recover $1.00$,
$12$--$15$ recover $0.96$, $14$--$19$ recover $0.38$, and $16$--$35$ recover
$0.11$, matching the single-layer-$16$ result in that run; the null arm is
$0.00$. On TheoremQA, the same windows yield $0.90,0.73,0.45,0.21$, with the
last again matching layer $16$ alone. The late loss of transfer is therefore not
resolved by writing at more late layers. This finding is compatible with late
reading that depends on bindings established earlier \citep{feng2024binding};
the interventions do not identify which such dependencies are responsible.

\paragraph{A spectral drop is not evidence of compression.}
The centred participation ratio appears to fall from $64$ at layer $12$ to
$17$ at layer $16$. Inspection reveals massive activations
\citep{sun2024massive, shi2026melayer}: in $34$ of $40$ skills, one token reaches
$99$--$180$ times the median span norm on the same two hidden dimensions.
Removing the four highest-norm positions from about four hundred restores the
ratio to $66$; with this correction it is approximately flat from layer $0$ to
$28$. The step is absent in Qwen3-0.6B and Qwen3-1.7B. We therefore retract the
compression interpretation: transfer declines without the measured loss of
dimensionality. Reporting per-position norm ratios (here median $1.6\to150$)
helps diagnose this failure of a spectral summary (Appendix
Table~\ref{tab:prdiag}).

\subsection{Skill identity, question dependence and token content}
\label{sec:whose}

The recovered effect is specific to more than a common shift or a state norm.
Averaging content across skills preserves $54$--$60\%$ of vector energy but
recovers only $0.11$ ($0.06$ with the skill last; pre-fix). Rescaling each patched
position to the receiver's norm leaves recovery unchanged at $1.06$ versus
$1.06$ (pre-fix). These controls support a role for the structured content of
the representation (Appendix~\ref{app:belongs}).

With the skill before the question, causal attention prevents its states from
encoding the later question: states are identical across a skill's twenty
instances at equal prompt length, and have cosine at least $0.997$ otherwise.
With the skill after the question, another instance's content still recovers
$0.96$ against $1.00$ for the instance's own at layer $8$ ($n=28$), while the
instance-specific remainder recovers $0.04$. At layer $16$, however, transfer
from another instance drops to $0.48$ against $0.85$, with the remainder alone
still at $0.04$. The clean separation of skill from question therefore holds at
the early injection layer; deeper states contain question-dependent information
that neither component transfers alone. Placing the skill last also extends
transfer to layer $16$ ($0.85$ on ten calculators, versus $0.19$ on forty with
the skill first), so prompt order affects the measured boundary.

Splitting the skill span into four contiguous quarters identifies a prose
description, formulas, tool signatures and a worked example. On forty
calculators ($n=60$), injecting these quarters separately recovers
$0.12,0.56,0.12,0.12$. The formula quarter carries the largest individual effect,
about two thirds of the full span's $0.87$; the remaining content contributes
through combination rather than a comparable standalone effect.

\subsection{Replication across models and task domains}
\label{sec:ladder}

We repeat span injection, depth sweeps and rank truncation with Qwen3-0.6B
($D=1024$) and Mistral-7B ($D=4096$, $32$ layers), and on TheoremQA. Injection
precedes each model's handover boundary, and receiver screening is recomputed
per run. Table~\ref{tab:replication} records which controls and knockouts are
available; Qwen3-1.7B, where skills rescue only $8$--$11\%$ of items, is reported
in Appendix Table~\ref{tab:laddermodels}.

\begin{table}[t]
\caption{Replication after the instrument fix. Injection layers are $8$ for
Qwen3-8B, $6$ for Qwen3-0.6B and $4$ for Mistral-7B. Recovery intervals bootstrap
skill groups. Max control is the largest available half-dose, other-skill,
permutation or noise result. Handover brackets the relative depth where
$\rho$ crosses below $0.5$; reading is the first knockout depth retaining at
least half of rescued items. $k^{*}$ is the grid-defined rank knee.
``--'' denotes a measurement not run after the fix.}
...
Span recovery ranges from $0.74$ to $0.94$ across all six combinations.
Available specificity controls remain at or below $0.27$, no more than a third
of full-span recovery. Wherever both depth sweeps are available, handover ends
before skill attention ceases to matter. Within Qwen, handover lies at relative
depth $0.33$--$0.43$; Mistral's is earlier at $0.19$--$0.22$. Reading boundaries
also depend on task: Qwen3-8B crosses at $0.56$ on TheoremQA and $0.78$ on
MedCalc, while Mistral crosses at $0.62$--$0.88$.

The pre-registered proportional-width prediction puts Qwen3-0.6B's rank knee
near $12$. Its observed recovery at $k=16,32,64,128$ is
$0.02,0.21,0.74,0.72$, giving $k^{*}=40$ [$32,50$] against Qwen3-8B's $52$
[$43,72$]. TheoremQA gives $52$ and $50$, respectively, also inconsistent with
proportional scaling. Participation ratios agree in scale ($66$--$77$ versus
$70$--$99$ over layers $0$--$20$, excluding massive activations). Mistral at the
same width as Qwen3-8B instead has $k^{*}=84$ [$59,97$] on MedCalc and $64$
[$50,79$] on TheoremQA. The difference is clearer on MedCalc; its $k=128$
recovery remains below untruncated recovery ($0.71$ versus $0.92$), so the tested
grid bounds the knee. Rank requirements depend on the model family as well as
the skill, and occupy about $4\%$ of Qwen3-0.6B's width versus $1.3\%$ of
Qwen3-8B's.

\subsection{Validity of the behavioural readout}
\label{sec:traps}

Three diagnostics explain why we report decoded accuracy with explicit controls
(Appendix~\ref{app:traps}). First, aggregate gold-answer log-probability can
reverse the accuracy ordering: a content-free mean vector exceeds the actual
skill state by $3.75$ nats while scoring $0.256$ versus $0.436$ accuracy; at
layer $21$, cells where the skill does not help contribute $133\%$ of the gap.
Second, multiple choice creates a floor: replacing every number in a skill
retains nearly three quarters of its effect in multiple choice and none in
free-form answering \citep{balepur2024artifacts}. Third, an irrelevant skill can
lift rescued-item accuracy from $0.025$ to $0.150$--$0.250$, making the wrong-skill
receiver distinct from no skill. A forced \texttt{ANSWER:} cue likewise removes
the span's behavioural effect while retaining a $+0.60$ nat difference, so it
cannot replace generation as the readout.

Additional domains show why these controls remain necessary. On LogicBench's
mostly binary answers ($n=57$), half dose and another skill recover $0.44$ and
$0.38$ against $0.75$ for transplantation, with no sharp depth transition;
CHAMP leaves only $20$ rescued items. ToolQA and BigCodeBench require tools or
sandboxed execution and are outside this no-tool evaluation. These observations
are retained in Appendix~\ref{app:replication}. None of the tested final-position
geometric measures distinguishes rescued items from failures, reinforcing the
need to evaluate representations through their behavioural effects.

\section{Conclusion}

A skill's behavioural effect is recoverable through representations on its own
token span, while a one-time intervention at the final prompt position primarily
transfers a compact answer decision in the tested tasks. Matched receivers
separate content from presence, and span controls show that recovery depends on
the correct content, positional arrangement and multiple leading directions.
Rank truncation measures this dependence directly: the observed rank knee does
not scale proportionally with width within Qwen, but changes across model
families. Successful transfer also ends earlier in depth than the need to attend
to the skill, without evidence that the span has collapsed into a low-dimensional
summary. Together, these findings connect where a skill is represented, how much
of that representation must be injected, and how much of its behavioural effect
can be recovered.
...
intervals from group-bootstrap uncertainty.
