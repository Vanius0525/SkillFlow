...
def fig_channels(dose, ctrl, diffvec, out: pathlib.Path):
    ...
    Every arm that injects at the last prompt position sits on the no-document
    baseline. The same content, written over the document's own token span,
    ...
    # drew the document below the transplant
    ...
    for ax, items, src, title, floor, floor_lab, gold in (
            (axT, last, dv, "written at the last prompt position "
             f"($n={len(dv.get('gold', []))}$)", base, "no document", gold),
            (axB, span, by, "written over the document's own span "
             f"($n={n}$)", recv, "filler document, unpatched", gold_span)):
    ...
    for ax, gl in ((axT, sum(dv["gold"]) / len(dv["gold"])), (axB, gold_span)):
        ax.text(gl - 0.012, -0.72, f"gold document ({gl:.2f})",
                fontsize=6.6, color="#009E73", va="center", ha="right")
    axB.set_xlabel("accuracy on the rescued items the document is needed for",
                   fontsize=8)
...
def fig_rank(paths, out: pathlib.Path, ladder=None, family=None):
    ...
    if family:
        ...
        # the document or of the model is decided here, not by the 0.6B curve
    ...
    # (0.87 / 0.89 / 0.92 on the three models), so this line is the document
    ax.text(130, 1.02, "the document in the prompt", fontsize=7,
