...
the same length (cosine $0.91$--$0.95$ from layer 0 to 10). Read naively that
...
across two reruns. Every span-channel number from \S\ref{sec:battery} on is
from runs after the fix unless marked pre-fix; the final-position and synthetic
results of \S\ref{sec:decomp} and the geometry predate it (Appendix~\ref{app:limits}). An identity intervention that
...
with every number replaced $0.23$--$0.31$, with its sentences shuffled
$0.19$--$0.29$---a factor of three to four, so ``the content component'' without a
...
nothing---replacing the final position repairs at most 3 of 18 rescued items (layer 10) and 0--1 from layer 14 on, and
...
because on the others the model does not need the document and a coherent
replacement lets it fall back on what it already knows; the
...
\emph{Transfer}: written over the prefix two documents share, another skill's
vector gives $0.19$ against $0.94$ for this skill's own vector on the same
positions ($n=16$), and on forty calculators $0.06$ against $0.73$ whether or not
the donor computes the same quantity, so the failure is the donor, not the
sub-span.
...
\paragraph{The split these are read on.} On six of the ten development
calculators the receiver---a wrong clinical document---already solves some item:
...
The span is $415$ to $1{,}630$ positions wide on the forty calculators, the same order as the answer it has
...
span at \emph{every} layer at once recovers $0.950$ (pre-fix), which makes ``one layer is
...
it does is an artefact.} The centred participation ratio of the span's content
matrix reads $64$ at layer $12$ and $17$ at layer $16$, as if the document were
...
ratio beside it (here $1.6 \to 150$, medians).
...
scaling is rejected on both tasks, and the content matrix's participation ratio
agrees ($66$--$77$ on Qwen3-0.6B, $70$--$99$ on Qwen3-8B over layers $0$--$20$,
massive activations excluded).
...
every skill and writing that average recovers $\rho=0.11$ ($0.06$ with the
document last; pre-fix) while accounting for $54$--$60\%$ of each vector's
energy. It is \textbf{not a magnitude}---rescaling per position to the
receiver's own norm changes nothing ($1.06$ against $1.06$; pre-fix). It is \textbf{the skill and
...
all items, at the last layer, the gold answer's log-probability puts a
content-free mean vector $3.75$ nats \emph{above} the state the model actually
builds while reading the document, with accuracy $0.256$ against $0.436$---a
flattened distribution never assigns $-19$ nats and never reaches the argmax; at
layer 21 the cells where the document does not work contribute $133\%$ of the
gap. \textbf{Multiple choice manufactures a floor}: a document with every number
replaced keeps nearly three quarters of the effect there and exactly none in free-form,
...
One direction is not enough, and neither are sixteen
(Figure~\ref{fig:rank}). On forty calculators after the fix ($n=60$), recovery
at layer $8$ is $0.13, 0.06, 0.13, 0.54, 0.77$ at $k = 4, 16, 32, 64, 128$
against $0.87$ untruncated, and half of the curve's range above $k=16$ is
recovered at $k^{*}=52$ ($95\%$ bootstrap interval over calculators $[43, 72]$;
...
against $0.87$) that makes little difference; where it is not (Mistral-7B,
...
question the restriction there is per item. Knockout rows are fractions of each
...
---and the per-item restriction selects items near the model's decision
boundary; we have not separated the two. (ii) The same per-item selection is the
...
answers the per-item split ($n=57$, $16$ skills) gives the transplant $0.75$
...
restriction buys is that the comparisons inside it are between $0.750$ and
at most $0.083$ rather than between numbers a confound could produce.
...
where the untruncated version recovers most of the effect, and the effect is
absent at $k \le 32$ while the bottom-$k$ control stays at the floor. That
...
the same content vector gives $1.00$ against a receiver at $0.00$ ($n=16$, four
calculators).
