...
| 5 | ~~GATE-W0 判据~~ | ~~`corr ≥ 0.99` 且 `MAD ≤ 0.05`~~ | **已处置（2026-09-10），见 §5c。** 实测之后：`corr` 不再进判据（它测的是序列长度），`MAD ≤ 0.05` 保留，另加 argmax 一致率 ≥ 0.99 |
...
## 5c. GATE-W0 的阈值：实测分布与处置（2026-09-10）

§5b 第 5 条把 `corr ≥ 0.99` 且 `MAD ≤ 0.05` 标为「阈值是拍的，先看实际分布再定」。
现在有分布了。

**第一次真跑，门槛拒掉 41% 的前向，而它们没有一个是错位的。**
gold 臂 142 次前向（Qwen3-8B bf16，生成与重放**都走 HF**，同一个 tokenizer、
同一个 chat template、同一个 eager attention —— 见 `howskill/hf_client.py`，
这一步本身就是为了把引擎差异消掉）：

| 量 | min | median | p95 | max |
|---|---|---|---|---|
| **MAD**（nats） | 0.00144 | 0.00575 | 0.00916 | **0.01704** |

**没有一条接近 0.05，连 0.02 都没有**，而且 **prompt digest 不匹配 0 条** ——
重放重建的 prompt 是逐字节正确的。失败组和通过组的 MAD **完全一样**
（都在 0.003–0.006），只有 `corr` 把它们分开。

**`corr` 分开它们，是因为 `corr` 测的是序列长度。** 同一批前向，按生成长度分组的
`corr` 第 5 百分位：

| 生成长度 | <150 | 150–250 | 250–400 | 400–600 | >600 |
|---|---|---|---|---|---|
| corr p05 | **0.9555** | 0.9634 | 0.9858 | 0.9864 | 0.9924 |

原因是结构性的：这些是**贪心选出来的 token** 的对数概率，所以大多数贴近 0，
序列本身方差很小。Pearson r 要除以这个方差，所以在短生成上**一个 token 抖动
0.005 nats**（bf16 噪声，和通过组带的是同一种噪声）就能把 r 压到 0.99 以下，
而 MAD 还停在 0.003。

**处置：**

1. **判据是 MAD。** `0.05` 保留 —— 它是实测最坏值的约 3 倍，而真正的错位
   （整体偏移一个 token）会让 MAD 动整个 nats 级，所以这个阈值仍然拦得住它。
2. **`corr` 降级为诊断量，不进判据。** 记录在案的用途是：
   **`corr` 很低 + MAD 很大 = 真错位**；`corr` 低而 MAD 小 = 算术噪声。
3. **新增 §3.5 要求过、代码里一直没有的那个检查：argmax 一致率** ——
   重放的 argmax 落在「当初真的生成出来的那个 token」上的比例。
   贪心解码下它应当 ≈ 1.0；它对让 logprob 比较变得棘手的那点算术噪声**不敏感**，
   而对错位**极敏感**（偏一位就会让它崩）。阈值 0.99。

代码在 `howskill/wb_replay.py`（`MAD_MAX` / `CORR_FLOOR_INFO` / `argmax_agreement`），
常量旁边写了上面这组实测数字，免得以后又被当成拍的。

**一条方法学教训值得带到别处**：一个「过严的门槛」和一个「坏掉的实验」在日志里
长得一模一样（都是一串 FAIL）。区分它们的唯一办法是**看被拒样本的原始量，
而不是看拒绝率**。这次如果直接按拒绝率去改 chat template 或 dtype，就会在一个
本来正确的重放上浪费若干轮。
