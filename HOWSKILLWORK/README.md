...
**GATE-1 / GATE-2 均已通过。方向已收窄为 1–2 步短任务 + 白盒机制，见 [`P8-WHITEBOX.md`](P8-WHITEBOX.md)。**
...
| P2 主效应 + H6 判定 | ✅ **GATE-2 通过**（33.6 → 77.3，gold−neutral +47.5pp） |
| P3 内容消融 | ⬜ 跑 |
...
CI 按 calculator 聚类 bootstrap（55 簇），所以区间宽是口径正确的结果：
...
- 存在效应（错配 − no_skill）= **−1.3pp** [−7.6, +5.7]，**干净的零**

**生成改成走 HF 而不是 vLLM**（`howskill/hf_client.py`，`--engine hf`）：
同一个 tokenizer、同一个 chat template、同一个 eager attention，
所以生成与重放的 token 流**按构造相同**，GATE-W0 从「引擎会不会不一致」
收窄成「重放的算术对不对」。代价是单流，`--workers` 被强制为 1。
