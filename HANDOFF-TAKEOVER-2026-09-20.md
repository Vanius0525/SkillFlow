...
**自变量维度（坐标轴没覆盖满）**
2. **秩曲线**：MedCalc 三个模型是密网格 k=1…256；**TheoremQA 三行仍是稀疏 5 点（k=4…128）**，
   且在 k=128 处远未回到未截断值（.67/.61/.76 vs .84/.74/.94）。补密网格约 9 GPU·h。
3. **Mistral×MedCalc 的秩曲线在 k=256 只有 .72（未截断 .84）**，需要 k=384/512 才看得到饱和，约 0.4 GPU·h。
4. **表 4 下半（问题依赖）只有第 8、16 两层**。补全 36 层 ≈ 64 GPU·h；粗网格（0/4/12/20/28）≈ 6 GPU·h。
5. **§4.1 的两个负对照（打乱句子 / 破坏数字）只跑了 14/28 层**（正文只引用 21–27 层区间）。补全约 2 GPU·h。
6. **表 1 的「补丁位置数」只有 1/4/16**（合成任务），没有更大的 k。补 64/256 约 4 GPU·h。
