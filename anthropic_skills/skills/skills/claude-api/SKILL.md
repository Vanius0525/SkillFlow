...
1. **Look at project files** to infer the language:
   ...
   - `*.py`, `requirements.txt`, `pyproject.toml`, `setup.py`, `Pipfile` → **Python** — read from `~/agent-harness/anthropic_skills/skills/skills/claude-api/python/`
   - `*.ts`, `*.tsx`, `package.json`, `tsconfig.json` → **TypeScript** — read from `~/agent-harness/anthropic_skills/skills/skills/claude-api/typescript/`
   - `*.js`, `*.jsx` (no `.ts` files present) → **TypeScript** — JS uses the same SDK, read from `~/agent-harness/anthropic_skills/skills/skills/claude-api/typescript/`
   - `*.java`, `pom.xml`, `build.gradle` → **Java** — read from `~/agent-harness/anthropic_skills/skills/skills/claude-api/java/`
   - `*.kt`, `*.kts`, `build.gradle.kts` → **Java** — Kotlin uses the Java SDK, read from `~/agent-harness/anthropic_skills/skills/skills/claude-api/java/`
   - `*.scala`, `build.sbt` → **Java** — Scala uses the Java SDK, read from `~/agent-harness/anthropic_skills/skills/skills/claude-api/java/`
   - `*.go`, `go.mod` → **Go** — read from `~/agent-harness/anthropic_skills/skills/skills/claude-api/go/`
   - `*.rb`, `Gemfile` → **Ruby** — read from `~/agent-harness/anthropic_skills/skills/skills/claude-api/ruby/`
   - `*.cs`, `*.csproj` → **C#** — read from `~/agent-harness/anthropic_skills/skills/skills/claude-api/csharp/`
   - `*.php`, `composer.json` → **PHP** — read from `~/agent-harness/anthropic_skills/skills/skills/claude-api/php/`
...
4. **If unsupported language detected** (Rust, Swift, C++, Elixir, etc.):
   ...
   - Suggest cURL/raw HTTP examples from `~/agent-harness/anthropic_skills/skills/skills/claude-api/curl/` and note that community SDKs may exist
   - Offer to show Python or TypeScript examples as reference implementations

5. **If user needs cURL/raw HTTP examples**, read from `~/agent-harness/anthropic_skills/skills/skills/claude-api/curl/`.
...
**CRITICAL: Use only the exact model ID strings from the table above — they are complete as-is. Do not append date suffixes.** For example, use `claude-sonnet-4-5`, never `claude-sonnet-4-5-20250514` or any other date-suffixed variant you might recall from training data. If the user requests an older model not in the table (e.g., "opus 4.5", "sonnet 3.7"), read `~/agent-harness/anthropic_skills/skills/skills/claude-api/shared/models.md` for the exact ID — do not construct one yourself.

A note: if any of the model strings above look unfamiliar to you, that's to be expected — that just means they were released after your training data cutoff. Rest assured they are real models; we wouldn't mess with you like that.

**Live capability lookup:** The table above is cached. When the user asks "what's the context window for X", "does X support vision/thinking/effort", or "which models support Y", query the Models API (`client.models.retrieve(id)` / `client.models.list()`) — see `~/agent-harness/anthropic_skills/skills/skills/claude-api/shared/models.md` for the field reference and capability-filter examples.
...
See `{lang}/claude-api/README.md` (Compaction section) for code examples. Full docs via WebFetch in `~/agent-harness/anthropic_skills/skills/skills/claude-api/shared/live-sources.md`.
...
For placement patterns, architectural guidance, and the silent-invalidator audit checklist: read `~/agent-harness/anthropic_skills/skills/skills/claude-api/shared/prompt-caching.md`. Language-specific syntax: `{lang}/claude-api/README.md` (Prompt Caching section).
...
→ Read `~/agent-harness/anthropic_skills/skills/skills/claude-api/shared/prompt-caching.md` + `{lang}/claude-api/README.md` (Prompt Caching section)

**Function calling / tool use / agents:**
→ Read `{lang}/claude-api/README.md` + `~/agent-harness/anthropic_skills/skills/skills/claude-api/shared/tool-use-concepts.md` + `{lang}/claude-api/tool-use.md`
...
2. **`~/agent-harness/anthropic_skills/skills/skills/claude-api/shared/tool-use-concepts.md`** — Read when the user needs function calling, code execution, memory, or structured outputs. Covers conceptual foundations.
...
7. **`~/agent-harness/anthropic_skills/skills/skills/claude-api/shared/prompt-caching.md`** — Read when adding or optimizing prompt caching. Covers prefix-stability design, breakpoint placement, and anti-patterns that silently invalidate cache.
8. **`~/agent-harness/anthropic_skills/skills/skills/claude-api/shared/error-codes.md`** — Read when debugging HTTP errors or implementing error handling.
9. **`~/agent-harness/anthropic_skills/skills/skills/claude-api/shared/live-sources.md`** — WebFetch URLs for fetching the latest official documentation.

> **Note:** For Java, Go, Ruby, C#, PHP, and cURL — these have a single file each covering all basics. Read that file plus `~/agent-harness/anthropic_skills/skills/skills/claude-api/shared/tool-use-concepts.md` and `~/agent-harness/anthropic_skills/skills/skills/claude-api/shared/error-codes.md` as needed.
...
3. **`~/agent-harness/anthropic_skills/skills/skills/claude-api/shared/live-sources.md`** — WebFetch URLs for current Agent SDK docs.
...
Live documentation URLs are in `~/agent-harness/anthropic_skills/skills/skills/claude-api/shared/live-sources.md`.
