...
- **本地有三个 commit 没推**（写这段时连不上 github）：`ed12599`（experiment.sh
  的执行位）、`2b4bc19`（`--smoke` 自带 `-smoke` 后缀）、`84091b8`（report.py 打
  准确率的同层对照）。**第一件事是把它们推上去再同步服务器。**
- **紧接着该做的**：取 e2 的**准确率对照曲线**（数据已在磁盘上，命令见 §12.3r (b)）。
  logprob 通道上 filler 比 real 恢复得还多，E2 因此不能作 H1/H2 的证据；
  acc 通道是不是也这样，决定 E2 能不能留在论文里。
