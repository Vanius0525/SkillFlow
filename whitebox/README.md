...
## 现在在哪里 / 下一步（2026-09-03）

**接手先读 `../HANDOFF-whitebox.md` §12.3r**，那里有完整的现状、可信度边界和
按顺序排好的下一步。这里只放最短的摘要：
...
- **代码已经在 `origin/master` 上**（`ed12599` / `2b4bc19` / `84091b8`），
  服务器已生效；本机仍然断网，文档类 commit 还压在本地，不挡任何一步。
- **然后是 Tier B**：先用 `patchcheck.py` 量 8B 的 bf16 误差，再决定
  `WB_TIERB_DTYPE`，然后接在同一个 RUN_ID 里跑 `--phase b`。
  **Tier B 的四条层扫描一次都没画过，这是当前最大的空白。**
- **三个已知缺陷，和精度无关**：E1 的顺序拆分符号相反（测到的是位置不是内容，
  别引用它的结论）、E7 的探针三个条件都是 1.00 包括 `no_skill`（没有意义）、
  `方向反了` 那条轴只有 2 道题在动。
