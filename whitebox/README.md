...
- **E2 的准确率判据已经出了结果（§12.3s）**：filler 停在无文档基线（10/39 vs 9/39），
  real 反超真放文档（18/39 vs 17/39）——**acc 通道分得开内容和在场，logprob 通道
  分不开，两条相反的判读都要报**。判据没覆盖的那一列：**别题向量在 acc 上恢复
  75%/87%**，所以补丁带的是这份 skill 的**共享**状态，不到逐题粒度。
  **还欠配对检验**（每个差值都是个位数题目）：`python e2_acc.py
  results/20260903-074552/e2-tierA results/20260903-074552/e2-tierA-k4`。
