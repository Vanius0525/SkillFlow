...
## 现在在哪里 / 下一步（2026-09-06）

**接手先读 `../HANDOFF-whitebox.md` §12.3x**，那里有完整的现状、可信度边界、
按顺序排好的下一步，以及一张「还需要确定或改进」的清单。这里只放最短的摘要：

- **Tier A 跑干净了**：`results/20260903-074552`，Qwen3-1.7B，**float32**，39 题，
  逐层 0..27。`末层恒等检查 = 1.000 [OK]` —— 追了两轮的补丁通路自证第一次通过。
- **E2 现在读层 21，不读峰值（§12.3t）**。层 17–20 前向被打坏、对照在那里一起虚高，
  而 acc 的 argmax 峰值(19)正落在里面。层 21（预先定死的 `best_layer`）上 real
  **同时**分得开中性文档（p=0.064/0.031）和别题向量（p=0.049）。
  §12.3s 的「别题向量恢复 75%/87%、不到逐题粒度」**已撤回**——那是层 19 的产物。
- **主结果是「skill 修好的 13 题」那条曲线**：中性文档的向量**每一层都修 0 题**，
  层 21 上真向量修 12/13、别题向量 4/13。比聚合准确率清楚得多。
- **Tier B 的 dtype 已经量过（journal 2026-09-04）**：bf16 误差比 1.7B 小一个数量级，
  但那是因为抽到的 8 题里 7 题 gold 就是 argmax；**fp32 下每层 0.00%、通路自证成立**，
  且加载没 OOM。下一步 `./tierb.sh --smoke --dtype float32` 定装不装得下。
- **E1 不是坏的，是功效不够**：位置项 +0.100 ≥ 内容项 +0.083，且两半是不同的题。
  `--order both` 已成默认（题内配对）。**但修完区间反而宽了 7 倍**，原因待查（§12.3x 第 5 节）。
- **两个新东西还没跑过**：`e2_patch.py` 现在记四个选项的完整 logprob（判 mean 的
  +5.44 是「统一抬升」还是「选择」），以及全新的 **E10 就地移植**（`e10_span.py`，
  把 skill 自己那段 token 的残差搬进等长对照 prompt —— 曲线掉下去的那一层就是
  「最后一层还能读到文档」）。
- **代码全部在 `origin/master`**（到 `fd4020e`）。
- **然后是 Tier B —— 当前最大的空白，四条层扫描一次都没画过**：
  `./tierb.sh --smoke --dtype float32` 定装不装得下（也是唯一的计时来源），
  再 `nohup ./tierb.sh --dtype float32 --full &`。
- **三个还没处置干净的**：E7 的探针（改成 `--probe answer` 了，**还没重跑**）、
  E7 的峰值层 27 可能是分母造成的（`||d||` 和 `||h||` 现在分开存了，重跑即判）、
  `方向反了` 那条轴只有 2 道题带这种干扰项（**论文里不写这条轴**）。
  `e0-tierA-num` 贴地板，已从阶段表下线。

`results/20260903-072820` 是垃圾（smoke 和真跑共用了 RUN_ID），可以删。
...
状态（2026-08-31）：**两梯都在新代码、新题集上跑完了** —— Tier A
`20260830-163838`（1.7B，39 题），Tier B v2 `20260830-170507`（8B，116 题）。
原始输出和逐条判断在
[`journal/2026-08-30-tierA-refit-tierB-margins.md`](journal/2026-08-30-tierA-refit-tierB-margins.md)，
结论在 HANDOFF §12.3o。四句话：

- **echo 陷阱确实是那 −0.165**：基线 0.085 → **0.231**（随机 0.25），Δacc
  +36.2 → **+20.5pp**。§12.3m 的两条预注册都命中。
- **但 Tier A 现在过不了自己的门槛**（Δacc CI [0.0, 41.0]，Δlogprob CI 含 0）。
  点估计不小，是 n=39 的区间太宽。**后果是 Tier A 上所有恢复率没有分母。**
  加题能解（`build.py` 里放宽 `for value in (2,3,4,5,6)`，实测能到 89 / 202 题），
  但 **2026-08-31 决定不加**，理由见 HANDOFF §12.3o —— 限制照实报，
  `report.py` 会自己判出「分母未确认」并打横幅。
- **Tier A 上的中性文档不是惰性的**：Δacc −7.7pp，却把金 token 推得比 skill
  **还远**（+2.194 CI 不含 0，skill +0.641 CI 含 0），同时把可解析率打到 85%。
  「内容占多少」在这一梯上**判不了**——两条通道给出相反的答案。
- **Tier B 给出了第一个带区间的预注册判词：只分离了一半**（constants 偏向自己
  那轴 +1.449 CI 不含 0；procedure +0.723 CI 含 0）。而 `e0-tierB-proc` 准确率
  是零、三条轴间距全显著为正 —— 换因变量的理由被直接看到了。

再下面那几段状态说的是**旧题集上的**结果，其中 Tier A 那部分已被 §12.3m 作废。
...
**这一小节写于 2026-08-30，已经跑过了** —— Tier A 的结果在
`results/20260903-074552`（fp32）。当前的下一步在本文件开头和 §12.3r，
不在这里。下面的命令模板仍然有效，照抄时把 `./experiment.sh` 用起来
（它把体检、跑、收图串成一条）。
...
| `filler` | `--filler` 打开。**同一题、同一层、同一位置**，只换掉捕获时上下文里是哪份文档。real 减 filler 才是"内容"那部分,剩下的是"上下文里有份长文档" |

后三条不是装饰。**缺了它们,恢复率这个数没法解释。**

### 两条通道，读哪一条

每一层同时记 `lp_*`（金答案 logprob，恢复率的来源）和 `ok_*`（生成后判对错）。
**它们会打架，而打架本身是结果。** Tier A 上 logprob 说 filler 恢复得比 real 还多
（+2.783 vs +1.470），准确率说 filler 一题没恢复（10/39，无文档基线 9/39）而 real
反超真放文档（18/39 vs 17/39）。判决和完整推导在 HANDOFF §12.3s。
...
整条对照曲线 + 配对 discordant 计数 + 精确 McNemar + 配对 bootstrap CI，
在峰值层和 summary 里那个 logprob 最佳层各算一遍（峰值层是在这条曲线上 argmax
挑的，同层做检验 p 偏乐观；后者在看这条通道之前就定死了）。**纯后处理**，
数据在 `per_layer.jsonl` 的 `rows` 里，不用重跑。

差值都是几题的量级时（39 题上 real 减 filler 只有 8 题），
**差值本身不是可读的量,配对计数才是**。

### 谁被补丁修好了 —— 聚合曲线答不了的那个问题

补丁 18/39、真放文档 17/39，可以是「同样的 17 题再加一题」，也可以是**两组几乎不
重叠、大小碰巧一样**的题。曲线分不开这两种，所以 `e2_acc.py` 还按 skill 自己对每道
题做了什么来分组：

| 组 | 无 skill → 有 skill | 它回答什么 |
|---|---|---|
| `fixed` | 错 → 对 | **行为效应就是由这组构成的**。补丁要是复现了效应,这一列该接近 1.0 |
| `never` | 错 → 错 | skill 都救不了的题。补丁在这里得分 = 补丁在做别的事 |
| `kept` | 对 → 对 | 本来就对。掉下去 = 补丁在破坏前向 |
| `broken` | 对 → 错 | skill 弄坏的题 |

外加一张 2×2：**无文档时答错的那些题里，补丁答对的和文档答对的是不是同一批**。
「补丁修的多数是文档修不好的题」意味着两边数目上的一致是巧合,不是机制上的一致。

两个读的时候要记住的：`fixed` 这组是**用有 skill 的结果定义的**，所以组内两个基线
按构造就是 0 和 1，只有补丁那几列有信息；同理组内的 logprob 增益偏乐观（选中的是
「有 skill 时恰好答对」的题）。Tier A 上这组只有 13 题，**读方向，不要读成比率**。
...
## E14：把注入的位移拆成「在场」和「内容」（2026-09-11 新增）

`e14_decomp.py`。E11 的超集。在最后一个 prompt 位置上，对每道题、每一层：

```
t_i = h_gold(i) − h_none(i)      总位移
g_i = h_filler(i) − h_none(i)    在场：等长的错文档做到的
d_i = h_gold(i) − h_filler(i)    内容：在场抵消之后剩下的
t_i = g_i + d_i                  恒等式
```

相对 E11 新增四件事：

1. **接收方可以换成 filler prompt**（`--no-filler-receiver` 关掉）。E11 只往「完全没文档」
   的前向里注入，于是「内容成分有效」和「接收方本来什么都没有」是混在一起的。
   注意 `h_fil + 1.0·d_i` 恒等于 `h_yes`，所以那一臂就是 E2 的 `real` 换了上下文。
2. **供体分同族 / 跨族**。E11 的 `add_diff_shift` 取 i+1，在 Tier A 里通常是同一个换算族。
   「可复用的 skill 向量」和「同一行表内可复用」是两个主张。
3. **逐题存几何**：‖d‖ ‖g‖ ‖t‖、三者夹角、`cos(dᵢ,dⱼ)` 分同族/跨族、去均值参与比，
   并带上四格标签（R/F/K/B），所以「被救回来的题的 d 有没有不一样」是离线 groupby。
4. **α 剂量反应**，和 **`--tail-k`**（补多少个位置）。后者在自由作答上决定了实验结论。

**读法：先读 R 格的准确率，再读 logprob。** `add_g` 在每一层都贴着基线，
是「可以把 d 叫作内容成分」的许可证；没有那一行，整个分解就只是一个定义。

### `--mode num`：不把题改写成选择题

`--gen-acc` 打开之后，num 模式用**贪心解码 + 数值判分**拿准确率（`genacc.py`）。
必须这样做的理由是实测的：同一批题、同一份 skill、同一个模型（Qwen3-1.7B），
选择题 0.180 → 0.300，自由作答 **0.000 → 0.000**；它拿到 skill 之后输出的是
**换算表里的因子**，不做那一步乘法。多选把「查表」记成成功，还额外送一个位置偏好的地板。

### 三份对照文档

| 文档 | 与 gold 的差别 | `d` 对它做减法之后剩下什么 |
|---|---|---|
| `tasks/filler-neutral.md` | 完全无关的等长文本 | 「这份文档是关于单位的」 |
| `tasks/tier_a/ctrl_shuffled/SKILL.zorb-units.md` | 同样的行，顺序打乱 | 「这份文档是连贯的」 |
| `tasks/tier_a/ctrl_corrupted/SKILL.zorb-units.md` | 同样的结构，**每个换算因子都换掉** | **「因子就是这几个数」** ——本任务能给出的最窄的内容定义 |

由 `tasks/tier_a/make_controls.py` 生成，会检查两件事：corrupted 里的数字不能撞上题的正确答案
（实测 7/249，2.8%），shuffle 不能是恒等。**三份都放在各自目录下、文件名相同**，
因为 `model.load_skill` 用文件名 stem 生成注入的 `# Skill: <name>` 标题 ——
叫 `SKILL.zorb-units.corrupted.md` 会把操作写进 prompt 里，对照就差了两件事而不是一件。

### 仪器自检

`genacc.first_token` + e14 的 self-test：**在最后一层**把补丁位置换成 `h_yes`，
那个位置的 logits 按构造就等于 skill 前向的，所以下一个 token 必须逐位相同。
这把「注入没带内容」和「注入根本没发生」分开 —— 后者在准确率曲线上长得像一个干净的零。

### 题集

`tasks/tier_a/tasks.large.jsonl`，n=358，`build_large.py` 生成（同一个生成器，
value 扩到 2–40）。冻结的 `tasks.jsonl`（n=39）**不动**，它是 2026-09-10 之前
所有结果的基准；大题集把 39 个设计单元全部重新生成，选项顺序独立重排。
